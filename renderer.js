// --- ADVANCED 8K RENDERING ENGINE v6.0 (PRO TYPOGRAPHY & NEW TOOLS) ---
...
window.PosterRenderer = function(ctx, W, H, appMode, formData, assets, positions, isExport = false) {
    ...
    try {
        ...
        if (appMode === 'discussion') {
            ...
            // Split screen handles normal tools AND the new Player Comparison
        ...
        if (appMode === 'motm_award') {
            ...
            grad.addColorStop(0, 'rgba(0,0,0,0)'); grad.addColorStop(0.6, 'rgba(0,0,0,0.85)'); grad.addColorStop(1, 'rgba(0,0,0,0.95)');
            ctx.fillStyle = grad; ctx.fillRect(0, 0, W, H);

            ctx.shadowColor = formData.primaryColor; ctx.shadowBlur = 40; ctx.shadowOffsetY = 0;
            ctx.fillStyle = '#ffffff'; ctx.font = '900 110px "Hind Siliguri", sans-serif'; ctx.textAlign = 'center';
            ctx.fillText("MAN OF THE MATCH", W/2, H - 360);
            ctx.shadowBlur = 0;

            drawPremiumGlassBox(40, H - 310, W - 80, 240, 30, 0.85, formData.primaryColor);
            
            ctx.shadowColor = 'rgba(0,0,0,0.8)'; ctx.shadowBlur = 10; ctx.shadowOffsetY = 5;
            ctx.fillStyle = formData.primaryColor; ctx.font = '900 85px "Hind Siliguri", sans-serif'; 
            ctx.fillText(formData.motmPlayerName, W/2, H - 210);
            
            ctx.fillStyle = '#cbd5e1'; ctx.font = 'bold 40px "Hind Siliguri", sans-serif';
            ctx.fillText(formData.motmTeam, W/2, H - 160);

            ctx.fillStyle = '#ffffff'; ctx.font = 'bold 45px "Hind Siliguri", sans-serif';
        ...
        // ==========================================
            ...
            drawPremiumGlassBox(30, 120, W/2 - 60, 110, 20, 0.85, formData.primaryColor);
            drawPremiumGlassBox(W/2 + 30, 120, W/2 - 60, 110, 20, 0.85, formData.primaryColor);
            
            ctx.shadowColor = 'rgba(0,0,0,1)'; ctx.shadowBlur = 10; ctx.shadowOffsetY = 4;
            ctx.fillStyle = formData.primaryColor; ctx.font = '900 65px "Hind Siliguri", sans-serif'; ctx.textAlign = 'center';
            ctx.fillText(formData.compPlayer1, W/4, 200);
            ctx.fillText(formData.compPlayer2, W*0.75, 200);

            ctx.shadowColor = 'rgba(0,0,0,0.5)';
            ctx.beginPath(); ctx.arc(W/2, 175, 60, 0, Math.PI*2); ctx.fillStyle = formData.primaryColor; ctx.fill();
            ctx.fillStyle = '#000'; ctx.font = '900 50px Arial'; ctx.shadowColor = 'transparent';
            ctx.fillText("VS", W/2, 192);
            ...
            for(let i=1; i<=4; i++) {
                ...
                drawPremiumGlassBox(W/2 - 140, startY, 280, 80, 40, 1, formData.primaryColor);
                ...
                ctx.fillStyle = '#ffffff'; ctx.font = '900 80px "Hind Siliguri", sans-serif'; 
                
                ctx.textAlign = 'right'; ctx.fillText(p1Stat, W/2 - 160, startY + 68);
                ctx.textAlign = 'left'; ctx.fillText(p2Stat, W/2 + 160, startY + 68);
        ...
        // ==========================================
            ...
            drawPremiumGlassBox(40, 150, W - 80, 220, 30, 0.9, formData.primaryColor);
            
            ctx.shadowColor = 'rgba(0,0,0,0.6)'; ctx.shadowBlur = 10; ctx.shadowOffsetY = 4;
            ctx.fillStyle = formData.primaryColor; ctx.font = '900 50px "Hind Siliguri", sans-serif'; ctx.textAlign = 'center';
            ctx.fillText("INNINGS BREAK - " + (formData.summaryTeamTitle || '').toUpperCase(), W/2, 220);
            
            ctx.fillStyle = '#ffffff'; ctx.font = '900 115px "Hind Siliguri", sans-serif';
            ctx.fillText(formData.summaryTeamTotal, W/2, 330);
            ctx.shadowColor = 'transparent';

            drawPremiumGlassBox(40, 420, W/2 - 60, H - 520, 30, 0.85);
            ctx.fillStyle = formData.primaryColor; ctx.font = '900 55px "Hind Siliguri", sans-serif'; ctx.textAlign = 'center';
            ctx.fillText("TOP BATSMEN", W/4 + 10, 500);
            
            ctx.strokeStyle = 'rgba(255,255,255,0.2)'; ctx.lineWidth = 3;
            ctx.beginPath(); ctx.moveTo(80, 530); ctx.lineTo(W/2 - 40, 530); ctx.stroke();
            ...
            drawPremiumGlassBox(W/2 + 20, 420, W/2 - 60, H - 520, 30, 0.85);
            ctx.fillStyle = formData.primaryColor; ctx.font = '900 55px "Hind Siliguri", sans-serif'; ctx.textAlign = 'center';
            ctx.fillText("TOP BOWLERS", W*0.75 - 10, 500);
            
            ctx.beginPath(); ctx.moveTo(W/2 + 60, 530); ctx.lineTo(W - 80, 530); ctx.stroke();
        ...
        // ==========================================
            ...
            const drawClashCard = (img, pos, x, y, w, h, color, isLeft) => {
                ...
                const slant = 35; 
                ctx.beginPath();
                if(isLeft) { ctx.moveTo(x, y + slant); ctx.lineTo(x + w, y); ctx.lineTo(x + w, y + h - slant); ctx.lineTo(x, y + h); } 
                else { ctx.moveTo(x, y); ctx.lineTo(x + w, y + slant); ctx.lineTo(x + w, y + h); ctx.lineTo(x, y + h - slant); }
                ctx.closePath();
                ctx.save(); ctx.clip(); ctx.fillStyle = formData.secondaryColor; ctx.fill();
                if (img) { ctx.translate(x + w/2 + pos.x, y + h/2 + pos.y); ctx.scale(pos.scale, pos.scale); ctx.drawImage(img, -img.width/2, -img.height/2); }
                ctx.restore(); 
                ctx.lineWidth = 5; ctx.strokeStyle = color; ctx.stroke();
                ctx.restore();
            };

            const cW = 220; const cH = 380; const cY = 320; 
            ...
            ctx.beginPath(); ctx.arc(W/2, cY + cH/2, 55, 0, Math.PI*2); ctx.fillStyle = formData.secondaryColor; ctx.fill();
        ...
        if (appMode !== 'h2h_3v3' && appMode !== 't_fixture') {
